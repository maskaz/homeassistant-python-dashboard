...
color = config_object["colors"]["cover"]
...
icon_on = config_object["icons"]["cover_on"]
icon_off = config_object["icons"]["cover_off"]
...
class CoverFrame(QWidget):
    ...
    def _update_gui(self, state_obj):
        ...
        position = 0 if position == 100 else 100 if position == 0 else position
    ...
    def slider_released(self, value):
        ...
        timer.timeout.connect(lambda eid=eid, position=position: self.send_slider_value(eid, position))
        self.debounce_timers[eid] = timer
        timer.start(300)

    def send_slider_value(self, eid, position):
        if position == 100:
            self.ha.call_service("cover", "close_cover", eid)
        elif position == 0:
            self.ha.call_service("cover", "open_cover", eid)
        else:
    ...
    def mousePressEvent(self, event):
        ...
        if self.on_frame_click and clicked_widget not in (self.slider, self.ico_button):
            ...
            state_obj = self.ha.entity_states.get(self.eid)
            if state_obj:
                current_state = state_obj.get("state")
                if current_state in ("open", "opening"):
                    self.ha.call_service("cover", "close_cover", self.eid)
                else:
                    self.ha.call_service("cover", "open_cover", self.eid)
